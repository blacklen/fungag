...
class PostSerializers(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if not attrs.get('title'): raise ParseError(
            {"error_code": '400_EMPTYD', "message": "Title khong duoc de trong"})
        if not attrs.get('image'): raise ParseError(
            {"error_code": '400_EMPTYD', "message": "Image khong duoc de trong"})
        if not attrs.get('category'): raise ParseError(
            {"error_code": '400_EMPTYD', "message": "Category khong duoc de trong"})
